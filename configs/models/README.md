...
**`MAX_MODEL_LEN`**, как и парсеры, задаёте **в пресете** (скопируйте пример и отредактируйте), ориентируясь на `config.json` / карточку HF и рецепты vLLM. До пресета: [`main.env`](../../main.env) (все несекретные дефолты), затем [`.env`](../../.env.example) **без** повторения имён из `main` (см. шапку `.env.example`), затем пресет.
