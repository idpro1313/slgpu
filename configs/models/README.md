...
**`MAX_MODEL_LEN`**, как и парсеры, задаёте **в пресете** (скопируйте пример и отредактируйте), ориентируясь на `config.json` / карточку HF и рецепты vLLM. До пресета подмешивается [`configs/main.env`](../main.env) (дефолты) и корневой `.env` — сильнее слой за слоем.
