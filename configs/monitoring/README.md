...
- Команда **`./slgpu monitoring up|down|restart`** вызывает `docker compose --env-file main.env` — **порты и bind** берутся из **`main.env` на диске** в корне репозитория. Если в web вы задали другие `PROMETHEUS_PORT`, `GRAFANA_PORT`, `LOKI_PORT` и т.д., а в `main.env` остались старые числа, контейнеры поднимутся **как в файле**, а не как в UI.
- **Задачи из web** (`native.monitoring.*`) передают compose **объединённый стек** из БД (`sync_merged_flat`) во временный env-файл — там порты из настроек **учитываются** (если вы перезапускаете мониторинг **через приложение**).
