...
- **`./slgpu monitoring`** и **native-задания** используют один файл **`${WEB_DATA_DIR}/.slgpu/compose-service.env`** (по умолч. **`data/web/.slgpu/…`**, под `data/`): в web он **заполняется из БД** (`sync_merged_flat`) перед `docker compose`; в bash — **копия `main.env`**, если он есть, иначе уже существующий снимок или **`docker/web-compose.defaults.env`**. Сервисы в compose ссылаются на этот путь в **`env_file:`** (подставляется **`WEB_DATA_DIR`** из того же снимка / `main.env`), а не на `main.env` в корне — отсюда **нет** требования иметь `main.env` на диске при работе только из UI.
- **Задачи из web** (`native.monitoring.*`, `native.proxy.*`) **всегда** пишут стек из БД в этот путь (доступен **slgpuweb** под `data/web`, без записи в `<repo>/.slgpu` в корне).
