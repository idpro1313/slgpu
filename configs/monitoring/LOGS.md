...
**Если Loki (или Promtail) падает с `read .../loki-config.yaml: is a directory`:** на хосте по пути `configs/monitoring/loki/loki-config.yaml` иногда оказывается **каталог** (при отсутствии файла Docker раньше мог создать пустой каталог с этим именем). Compose в репо монтирует **каталог** `loki/` → `/etc/loki` (аналогично `promtail/`). Удалите ошибочный каталог и восстановите файл: `rm -rf configs/monitoring/loki/loki-config.yaml` (если это каталог), `git checkout -- configs/monitoring/loki/loki-config.yaml`, затем **`./slgpu monitoring up`** (с обновлённым `docker/docker-compose.monitoring.yml`).
