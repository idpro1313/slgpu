...
- Ретенция и лимиты Loki: [`loki/loki-config.yaml`](loki/loki-config.yaml) (`retention_period` и др.). Для **`/loki/api/v1/query_range`** важен **`limits_config.max_entries_limit_per_query`**: дефолт Loki — **5000 строк**; без поднятия лимита запрос с `limit=8000` (как у «Отчёты логов» в web) даёт **400**. В шаблоне slgpu задано **25000**; после изменения нужен **`native.monitoring` restart/up** чтобы перерендерился `${WEB_DATA_DIR}/.slgpu/monitoring/loki-config.yaml`.
