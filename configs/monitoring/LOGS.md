...
**Если `monitoring fix-perms` падает с `sudo: command not found` (типично из web-UI):** в web-контейнере нет `sudo` и нет root-доступа после `runuser → slgpuweb`. Скрипт [`scripts/monitoring_fix_permissions.sh`](../../scripts/monitoring_fix_permissions.sh) выполняет `mkdir -p` и `chown -R` через короткоживущий root-контейнер (`docker run --rm --user 0:0 -v ...`) — это работает и на хосте от обычного пользователя, и из web (через `docker.sock`). Образ-помощник: переменная **`SLGPU_FIXPERMS_HELPER_IMAGE`** (по умолчанию `alpine:latest`); при оффлайн-стенде задайте уже доступный локально образ с `sh` и `chown`.
