...
Репозиторий поднимает **Loki** и **Promtail** вместе с Prometheus/Grafana: [`docker-compose.monitoring.yml`](../docker-compose.monitoring.yml), данные Loki на хосте: **`LOKI_DATA_DIR`** (по умолчанию `/opt/mon/loki`), позиции Promtail: **`PROMTAIL_DATA_DIR`** (например `/opt/mon/promtail`). Подъём: **`./slgpu monitoring up`**, права: **`./slgpu monitoring fix-perms`**.

- Ретенция и лимиты Loki: [`monitoring/loki/loki-config.yaml`](loki/loki-config.yaml) (`retention_period` и др.).
