...
3. **vLLM** — поиск на grafana.com по `vllm` (ID зависят от версии; импорт через **Dashboards → Import**). В репозитории: [`vllmdash2.json`](grafana/provisioning/dashboards/json/vllmdash2.json) — **V2**; в копии для slgpu все datasource указывают на **`uid: prometheus`** (как в provisioning), без плейсхолдера `${DS_PROMETHEUS}`. Разметка перенесена в SGLang-дашборд ниже.
