...
def preset_env_values(preset: Preset) -> dict[str, str]:
    """Canonical `.env` key/value payload for a DB preset."""
...
def render_preset_env_text(preset: Preset, *, client_download: bool = False) -> str:
    reason = "client download" if client_download else "preset export"
    return render_env_text(
        preset_env_values(preset),
        header=(
            f"Generated by slgpu-web for preset '{preset.name}'.\n"
            f"Source of truth: web UI. Reason: {reason}."
        ),
    )
