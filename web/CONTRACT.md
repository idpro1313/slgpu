...
| `presets` | Декларация пресета в БД (имя, HF ID, **движок** `vllm`/`sglang`, model_id, параметры key/value, GPU mask, путь к синхронизированному `.env`). При **экспорте** в `data/presets/<slug>.env` в файл дописывается **`SLGPU_ENGINE`** (согласовано с полем `engine`); импорт с диска сначала читает `SLGPU_ENGINE`, иначе — эвристика по переменным (например `SGLANG_MEM_FRACTION_STATIC` → sglang). UI умеет просматривать, параметрически редактировать и удалять запись; запись в файл — отдельный экспорт, удаление `.env` — отдельная подтверждаемая операция. |
