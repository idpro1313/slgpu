...
| `settings` | Ключ `public_access`: JSON с **`server_host`** (для ссылок в UI) и двумя секретами LiteLLM: **`litellm_master_key`** для proxy/Admin UI (при `native.proxy.*` становится derived env **`LITELLM_MASTER_KEY`** в `${WEB_DATA_DIR}/.slgpu/compose-service.env`, хотя не входит в реестр `stack_params`) и **`litellm_api_key`** для backend Bearer-вызовов `/v1` (отчёты логов). В ответах API значения **не** возвращаются, только флаги **`litellm_master_key_set`** / **`litellm_api_key_set`**. Плюс **`cfg.meta`**, унаследованные пустые **`cfg.stack`** / **`cfg.secrets`**. |
...
- **8.2.10:** разделены секреты LiteLLM: `litellm_master_key` → `LITELLM_MASTER_KEY` для proxy, `litellm_api_key` → Bearer для backend `/v1`-запросов.
