...
| `settings` | Ключ `public_access`: JSON с **`server_host`** (для ссылок в UI) и опциональным **`litellm_api_key`** (тот же смысл, что **`LITELLM_MASTER_KEY`** у прокси; в ответах API **не** возвращается, только флаг **`litellm_api_key_set`**). При `native.proxy.*` ключ становится derived env **`LITELLM_MASTER_KEY`** в `${WEB_DATA_DIR}/.slgpu/compose-service.env`, хотя не входит в реестр `stack_params`. Плюс **`cfg.meta`**, унаследованные пустые **`cfg.stack`** / **`cfg.secrets`**. |
...
- **8.2.9:** `public_access.litellm_api_key` сохраняется как derived `LITELLM_MASTER_KEY` в `compose-service.env` для proxy-стека, не возвращаясь в таблицу `stack_params`.
