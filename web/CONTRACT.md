...
| `jobs` | Долгие операции: **`native.*`** (stdout/stderr tail при **running**) и **`web.log_report.generate`** / **`web.log_export.generate`** (in-process: Loki + LLM HTTP или только Loki → файл; для них **`stdout_tail`** не пополняется). Поля: команда/`kind`, статус, exit code, tail, correlation id, инициатор. Для **`native.*`** пока job **running** периодически обновляются **`stdout_tail`** и **`message`** (последняя значимая строка), в т.ч. линии **stream docker pull** при **`native.slot.up`** (до старта контейнера). |
| `audit_events` | Действия: при `jobs.submit` — запись **с** `correlation_id` (дублирует job для трассировки). Отдельно — **только UI** (`correlation_id IS NULL`): пресеты, модель в реестре, настройки public-access. Лента «Задачи» строится из `GET /activity`: `jobs` + UI-`audit` без дубля CLI-строк. SPA **`/jobs`**: опционально выгрузка до **500** записей ленты и JSON выбранной `job` (включая `stdout_tail`/`stderr_tail`) в файл на ПК через браузер — без новых API. |
...
| `stack_params` | Плоский стек: `param_key`, `param_value`, `is_secret` (секреты маскируются в `GET /app-config/stack`). **`LITELLM_MASTER_KEY`** и **`LITELLM_API_KEY`** — обычные секреты блока **8. Секреты приложения**; режим `reveal_secrets=true` раскрывает их вместе с остальными секретами. Опционально **`LOG_REPORT_LLM_API_KEY`** — Bearer для внешнего LLM при **`LOG_REPORT_LLM_API_BASE`** (см. § про отчёты логов). |
...
| `log_reports` | **Отчёты по контейнерным логам из Loki** (UI «Отчёты логов»): период, scope/LogQL, лимиты, связь с **`jobs`**, детерминированный **`facts`** (JSON), итоговая **`llm_markdown`** (RU) из LLM (`POST …/v1/chat/completions` — по умолчанию внутренний LiteLLM, иначе **`LOG_REPORT_LLM_API_BASE`**) или локального fallback по `facts`, если LLM вернул ошибку; при сбое Loki/периода — `failed` + `error_message`. Сырые логи в БД **не** хранятся целиком. |
| `log_exports` | **8.6+:** полная выгрузка строк Loki в файл: период, **`logql`**, **`redact_secrets`**, статус, **`job_id`**, путь артефакта (относительно **`WEB_DATA_DIR`**), **`line_count`**, **`byte_size`**, заметка о retention, **`error_message`**. |
...
**`native.*`** — subprocess / docker-py / compose; **`web.log_report.generate`** — in-process пайплайн Loki → факты → LLM HTTP `/v1/chat/completions`; **`web.log_export.generate`** — Loki → полный **ndjson.gz** (без `./slgpu`).
...
| kind | Назначение |
|---|---|
| `web.log_report.generate` | По **`report_id`** (и **`job_id`**): запрос диапазона в **Loki** (`LOKI_*`), сбор **детерминированного пакета фактов**, best-effort вызов **`POST …/v1/chat/completions`** (русская Markdown-сводка) — базовый URL из **`LOG_REPORT_LLM_API_BASE`** или внутренний LiteLLM, ключ **`LOG_REPORT_LLM_API_KEY`** с fallback на **`LITELLM_API_KEY`** при кастомной базе, иначе только **`LITELLM_API_KEY`**; запись результата в **`log_reports`**. Если LLM недоступен или возвращает 5xx, job завершается успешно с локальной Markdown-сводкой по `facts` и предупреждением в `error_message`; при недоступности Loki или некорректном периоде/LogQL — **`failed`**. |

#### `jobs.kind` = `web.log_export.generate`

| kind | Назначение |
|---|---|
| `web.log_export.generate` | По **`export_id`** в **`Job.args`**: исчерпывающий **`query_range`** в **Loki** за **[`time_from`,`time_to`]** с постраничным **`limit`** (технический размер страницы **25 000**, как у Loki в шаблоне slgpu) и **рекурсивным делением** временного окна, пока страница «насыщена»; **без** LLM и без бизнес-лимита числа строк. Результат — **gzip JSONL** (`ndjson.gz`) в **`${WEB_DATA_DIR}/.slgpu/log-exports/`**, метаданные в таблице **`log_exports`**. Опционально **`redact_secrets`** — те же паттерны, что у отчёта, **без усечения длины строки**. Максимальный период запроса на стороне API согласован с **`limits_config.retention_period` / `max_query_lookback`** (в шаблоне **2880 ч**). Отмена job — прерывание пайплайна (**`failed`** у экспорта с пояснением). |

`CliCommand.argv` для **native.\*** и **web.\*** из UI **пустой**; параметры в **`Job.args`**. Для отчётов логов: **`scope`/`logql`**, **`time_from`/`time_to`**, **`max_lines`**, **`llm_model`**, **`report_id`**. Для выгрузки: **`export_id`**, расширенные фильтры (container, compose\_\*, slgpu\_\*) задаются в строке **`log_exports`**, не в job args. **v8.2.0+:** **`native.*`** и **`web.log_report.generate`**; **v8.6.0+:** **`web.log_export.generate`**; устаревший subprocess `./slgpu` для jobs снят.

API (все публичные ручки под префиксом **`/api/v1`**): `GET /api/v1/dashboard` — метрики БД, runtime, пробы сервисов и объект **`host`** (ОС/ядро/архитектура, hostname, CPU, RAM, диск по `WEB_SLGPU_ROOT`, NVIDIA+CUDA). Железо **хоста**: при доступном Docker socket backend делает `docker run` с bind-mount хостовых `/proc` и `/etc` (образ `WEB_DOCKER_HOST_PROBE_IMAGE`, по умолчанию `busybox`); при **`HOST_GPU_DOCKER_PROBE≠off`** — отдельный запуск **`nvidia-smi`** в образе из **`NVIDIA_SMI_DOCKER_IMAGE`/`WEB_NVIDIA_SMI_DOCKER_IMAGE`** с `device_requests` GPU (нужен **NVIDIA Container Toolkit** на сервере). Без Docker, при **`HOST_GPU_DOCKER_PROBE=off`**, или при сбое — чтение из namespace процесса web и локальный `nvidia-smi`. Конфиг приложения: `GET/PATCH /api/v1/app-config/stack`, `POST /api/v1/app-config/install`, `GET /api/v1/app-config/status`. Бенчмарк: `GET /api/v1/bench/runs`, `GET /api/v1/bench/runs/{engine}/{ts}/summary`, `POST /api/v1/bench/scenario`, `POST /api/v1/bench/load`; в UI результаты прогона — модалка по `summary.json`. Страница подставляет **движок** и **slug пресета** из `GET /api/v1/runtime/snapshot`, когда в snapshot есть активный запуск с известным пресетом.
...
- **UI «Логи» (журнал slgpu-web), read-only (5.1+):** **`GET /api/v1/app-logs/events`** — список структурированных событий из таблицы **`app_log_event`** (фильтры: `level`, `event_kind` (CSV), `path_prefix`, `q`, `since` ISO, курсор `before_id`, `limit` 1..1000). **8.5.0:** на SPA **`/app-logs`** кнопка сохраняет в JSON на ПК все строки по тем же фильтрам (клиент запрашивает страницы с `before_id`, с лимитами на объём выгрузки). **Опционально** JSON в файл: **`${WEB_DATA_DIR}/.slgpu/app.log`** только при **`WEB_LOG_FILE_ENABLED=true`** (Loki/дебаг). `DbLogHandler` дублирует поток `logging` (stdout) в БД. Middleware: `X-Request-ID`, `extra` с `path`/`status`/`request_id` для `app.http`. Пропуск access: `GET /healthz`, `/assets/*`, фавиконки, **`/api/v1/app-logs/events`**. **`uvicorn.access`** → WARNING. Страница SPA: **`/app-logs`**. Секреты в поля `log_extra` не кладутся; тела HTTP не пишутся.
- **UI «Отчёты логов» (8.2+):** **`POST /api/v1/log-reports`** — период (ISO **`from`**/**`to`**), **`scope`** (`slgpu` \| `all` \| **`custom:`+LogQL**), опционально **`query`**, **`max_lines`** (до 20 000 на стороне API; **≤ `limits_config.max_entries_limit_per_query`** у Loki — в шаблоне slgpu **25 000**; при старом Loki (**5000**) backend повторит запрос с **`limit=5000`**), **`llm_model`**; **`202 Accepted`** → **`report_id`**, **`job_id`**, polling **`GET /api/v1/log-reports/{id}`** (статус, **`facts`**, **`llm_markdown`**). **`GET /api/v1/log-reports`** — последние отчёты. **`GET /api/v1/log-reports/llm-catalog-source`** — **`use_litellm_model_catalog`** (нужен ли каталог **`/litellm/models`** на странице). **8.6+ — полная выгрузка в файл:** **`POST /api/v1/log-exports`** (период, **`scope`**, опционально **`logql`**, фильтры **`container` / `compose_*` / `slgpu_*`**, **`redact_secrets`**), **`GET /api/v1/log-exports`**, **`GET /api/v1/log-exports/{id}`**, **`GET /api/v1/log-exports/{id}/download`** — **`application/gzip`**, **`Content-Disposition: attachment`**. Job **`web.log_export.generate`**. Требует **поднятого мониторинга** (Loki reachable с web-контейнера по **`LOKI_SERVICE_NAME`**/**`LOKI_INTERNAL_PORT`**). Вызов Loki **`/query_range`**: только **`direction=backward`** или **`forward`** в **нижнем регистре**. Retention Loki в шаблоне мониторинга — **120 дней** (**`retention_period: 2880h`**, **`max_query_lookback: 2880h`**); после изменения лимитов или ретенции Loki нужен **перезапуск мониторинга**, чтобы попал отрендеренный `loki-config.yaml`. В LLM уходит только сжатый пакет фактов (**редактирование** секретных паттернов); при HTTP 500/недоступности LLM отчёт не падает, а получает локальную Markdown-сводку по тем же `facts`.
...
- **8.5.0:** SPA «Логи» (`/app-logs`) — выгрузка событий `app_log_event` в JSON на ПК (те же фильтры, что список; пагинация на клиенте). SPA «Задачи» (`/jobs`) — выгрузка ленты `GET /activity?limit=500` и выбранной записи `job` (хвосты stdout/stderr) в JSON на ПК; новых маршрутов API нет.
- **8.5.1:** Loki в шаблоне мониторинга хранит данные **120 дней**: **`retention_period: 2880h`** и **`max_query_lookback: 2880h`**; после обновления нужен перезапуск мониторинга для перерендера `loki-config.yaml`.
- **8.4.0:** отдельные **`LOG_REPORT_LLM_API_BASE`** / **`LOG_REPORT_LLM_API_KEY`** для OpenAI-compatible сводки «Отчётов логов»; **`GET /api/v1/log-reports/llm-catalog-source`**; пустая база — прежний путь через внутренний LiteLLM.
...
- **8.2.6:** отчёты логов больше не падают целиком при HTTP 500/недоступности LLM-сводки: `web.log_report.generate` сохраняет `facts`, пишет локальную Markdown-сводку через `render_fallback_markdown` и оставляет предупреждение в `error_message`.
