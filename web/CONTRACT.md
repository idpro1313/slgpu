...
Полный перечень ключей, групп, `allow_empty` и `required_for` — в **`GET /api/v1/app-config/stack`** (поле **`registry`**) и в **`STACK_KEY_REGISTRY`**. При **`registry[].ui_hidden: true`** строка не отображается в UI «Настройки», а в теле **`stack`** ответа ключ опускается (`presentation_stack`); значение для compose/слотов восстанавливается из остальных переменных (например внутриконтейнерный listen LLM из `LLM_API_BIND`/хост-портов через `sync_merged_flat`). Для **`group`** `monitoring` и `proxy` в **`registry[]`** задаётся **`subgroup`** (slug сервиса); UI группирует строки таблицы и рисует подзаголовки (**6.0.5**). **С 6.1.4:** в подгруппе **Langfuse** порядок ключей следует **`configs/main.env`** (§7 прокси): сначала сетевые поля **web**, затем **worker**, затем **`NEXTAUTH_URL`**, чтобы внутренний порт и bind не разъезжались в таблице. **С 6.1.5:** в подгруппе **MinIO** — **API** (`MINIO_API_*`) и **Console** (`MINIO_CONSOLE_*`): внутренний порт, bind и хост-порт идут подряд; **`MINIO_ROOT_*`** — после сетевых ключей. **С 6.1.6:** **`LITELLM_MASTER_KEY`** имеет **`ui_hidden: true`** — пользователь задаёт ключ в блоке «Внешний доступ», без второй строки в таблице параметров стека (источник для compose при наличии **`litellm_api_key`** — см. **`stack_config._merge_public_access_litellm_master_key`**). **С 6.1.3:** scope **`fix_perms`** в **`required_for`** не размазан по всему стеку: в реестре он только у ключей, которые реально читает **`native.monitoring.fix-perms`** (каталоги данных Grafana/Prometheus/Loki/Promtail/Langfuse-хранилищ, образы для uid/`chown`, **`SLGPU_BENCH_CHOWN_IMAGE`**), чтобы в колонке «обязателен для» не светился **fix-perms** у сотен параметров без отношения к `chown`.
