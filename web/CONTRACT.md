...
[`docker/docker-compose.proxy.yml`](../docker/docker-compose.proxy.yml)); опционально bash CLI
...
| `presets` | Декларация пресета в БД (имя, HF ID, model_id, параметры key/value для `.env`, GPU mask, путь к файлу `.env`). **Движок** (vLLM/SGLang) в пресете **не** хранится. Экспорт в `data/presets/<slug>.env` не добавляет `SLGPU_ENGINE`. **`POST /presets/sync`** и **`POST /presets/import-templates`** — импорт с `PRESETS_DIR` / копирование шаблонов из **`examples/presets`** (для хоста и автоматизации; **в SPA не вызываются**). **UI:** CRUD, клон; **выгрузка в `.env`** только из карточки пресета → **`POST /presets/{id}/export`**. Удаление из БД и, опционально, файла — отдельно. |
...
- Имена стеков Compose для **опроса Docker** (`com.docker.compose.project`):
  по умолчанию `slgpu` (инференс), `slgpu-monitoring` (метрики/логи), `slgpu-proxy`
  (Langfuse + LiteLLM); при другом `COMPOSE_PROJECT_NAME` задайте `WEB_COMPOSE_PROJECT_INFER`,
