...
| `jobs` | Долгие операции: **`native.*`** (stdout/stderr tail при **running**) и **`web.log_report.generate`** (in-process: Loki + LLM HTTP; для него **`stdout_tail`** не пополняется). Поля: команда/`kind`, статус, exit code, tail, correlation id, инициатор. Для **`native.*`** пока job **running** периодически обновляются **`stdout_tail`** и **`message`** (последняя значимая строка), в т.ч. линии **stream docker pull** при **`native.slot.up`** (до старта контейнера). |
...
**`native.*`** — subprocess / docker-py / compose; **`web.log_report.generate`** — in-process пайплайн Loki → факты → LLM HTTP `/v1/chat/completions` (без `./slgpu`).
