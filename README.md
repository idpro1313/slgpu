...
| **`bench`** | **Нагрузочный тест** против уже поднятого API на `127.0.0.1:8111`: запускает [`scripts/bench_openai.py`](scripts/bench_openai.py), подгружает пресет **`-m`** (для `MAX_MODEL_LEN`, `BENCH_MODEL_NAME` и т.д.), пишет артефакты в `bench/results/<engine>/<timestamp>/`. Engine **автоматически определяется** из `docker compose ps`, если не указан в аргументах. Проверяет соответствие запущенного engine и model из `/v1/models`; при несоответствии — ошибка с подсказкой. |
| **`load`** | **Длительный нагрузочный тест** (15–20 мин, 200–300 виртуальных пользователей): запускает [`scripts/bench_load.py`](scripts/bench_load.py), эмулирует фазы ramp-up → steady → ramp-down, собирает time-series метрики (throughput, TTFT, latency, error rate) в CSV каждые 5 сек. Артефакты: `summary.json`, `time_series.csv`, `users.jsonl`. Опции: `--users`, `--duration`, `--ramp-up`, `--ramp-down`, `--think-time`, `--max-prompt`, `--max-output`, `--report-interval`, `--burst` (макс throughput без пауз). Engine **автоматически определяется** из `docker compose ps`. **Автоматически проверяет** запущенный engine и model. |
