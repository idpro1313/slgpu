...
| **`web`** | **`up` / `down` / `restart` / `logs` / `build` / `install`**: приложение **Develonica.LLM**, образ **slgpu-web** ([`docker/docker-compose.web.yml`](docker/docker-compose.web.yml)), сеть `slgpu`; тома `data/web`, `data/models` по [`main.env`](main.env). Репо bind-монтируется в контейнер по **тому же абсолютному пути, что и на хосте** (`SLGPU_HOST_REPO`, экспортируется из `scripts/cmd_web.sh` как `$(pwd)`). **`install`** — только при **уже запущенном** `slgpu-web`: `POST /api/v1/app-config/install` импортирует `main.env` (+ опционально `configs/secrets/*.env`) в SQLite; порядок: **`./slgpu web up`**, затем **`./slgpu web install`**. |
