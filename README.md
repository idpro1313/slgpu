...
| **OOM при загрузке MoE (Kimi-K2.6 и др.)** на 4×~140 GiB | В логе: `create_weights` / `torch.empty` / не хватает ~1.3 GiB при ~137 GiB занято — часто **упор в размер весов на шард**, а не в KV. Понизить **`GPU_MEM_UTIL`** (резерв KV), **`MAX_MODEL_LEN`**, в пресете **`VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS=0`**; **`PYTORCH_ALLOC_CONF`** в `configs/vllm/args.env`. Если не помогает — **TP=8** или другой чекпоинт на HF |
