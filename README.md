...
| **vLLM:** `RuntimeError: Unexpected error from cudaGetDeviceCount()… Error 803: system has unsupported display driver / cuda driver combination` | Драйвер NVIDIA на **хосте VM** старше CUDA в образе пресета (например `qwen3_5-…-cu130` требует драйвер для **CUDA 13**). Варианты: **обновить драйвер хоста** до `≥ 580` (под CUDA 13.x), либо в пресете указать **`VLLM_DOCKER_IMAGE`** с младшим CUDA (`*-cu128`/`*-cu124` — если такой тег есть для семейства модели). Проверка хоста: `nvidia-smi` (строка `Driver Version` / `CUDA Version`). |
