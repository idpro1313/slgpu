...
> **Версия 8.1.6:** **Prometheus** — скрейп всех vLLM-слотов с разными **хост-портами**. Раньше job `vllm` брал только **`LLM_API_PORT`** из стека (один таргет `host.docker.internal:8111`); у мультислотов из UI у каждого слота свой порт (**8112, 8113, …**) — они не попадали в Prometheus автоматически. Добавлен job **`vllm-slots`** с **`file_sd_configs`** на **`${WEB_DATA_DIR}/.slgpu/monitoring/vllm-slots.json`**: файл при первом рендере мониторинга создаётся пустым `[]`, **не перезаписывается** backend'ом ; пользователь перечисляет порта в JSON (эталон: [`configs/monitoring/prometheus/vllm-slots.json.example`](configs/monitoring/prometheus/vllm-slots.json.example)). Relabel **`instance=vllm:<порт>`** как у основного job. Grafana: использовать **`job=~"vllm|vllm-slots"`**. Подробности — [`configs/monitoring/README.md`](configs/monitoring/README.md).
>
