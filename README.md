...
> **Версия 8.5.1:** Loki в шаблоне мониторинга хранит контейнерные логи **120 дней**: **`retention_period: 2880h`** и **`max_query_lookback: 2880h`** в [`configs/monitoring/loki/loki-config.yaml.tmpl`](configs/monitoring/loki/loki-config.yaml.tmpl). После обновления репозитория перезапустите мониторинг из UI (`native.monitoring.restart/up`), чтобы перерендерился `${WEB_DATA_DIR}/.slgpu/monitoring/loki-config.yaml`.
>
> **Версия 8.2.12:** параметры SGLang расширены **по каналу пресетов**: ключи **`SGLANG_DOCKER_IMAGE`**, **`SGLANG_DP_SIZE`**, **`SGLANG_ENABLE_DP_ATTENTION`** / **`_LM_HEAD`** / **`SGLANG_MM_ENABLE_DP_ENCODER`**, **`SGLANG_CHUNKED_PREFILL_SIZE`**, блок **EAGLE** (`SGLANG_SPECULATIVE_*`, **`SGLANG_ENABLE_MULTI_LAYER_EAGLE`**, **`SGLANG_ENABLE_SPEC_V2`**) и смежные **`SGLANG_TRUST_REMOTE_CODE`**, **`SGLANG_ENABLE_METRICS`** / **`MFU`**, **`NVIDIA_VISIBLE_DEVICES`** сохраняются в **`presets.parameters`**, импортируются из `.env` и участвуют в экспорте; на странице **«Пресеты»** они добавлены в подсказки **`COMMON_PARAMETERS`**; в реестре стека появились описания этих ключей. Ранее неизвестные ключи отбрасывались при нормализации пресета и терялись при импорте из файла — это блокировало эталон [`examples/presets/mimo-v2.5-8.env`](examples/presets/mimo-v2.5-8.env).
>
> **Версия 8.2.11:** ключи LiteLLM перенесены в **«Настройки → Стек в базе данных → 8. Секреты приложения»**: **`LITELLM_MASTER_KEY`** запускает LiteLLM Proxy/Admin UI, **`LITELLM_API_KEY`** используется backend-запросами `/v1/chat/completions` для анализа логов. Оба секрета подчиняются общему переключателю **«Показывать секреты в явном виде»**.
>
> **Версия 8.2.10:** ключи LiteLLM разделены по ролям: **`LITELLM_MASTER_KEY`** — master key LiteLLM Proxy/Admin UI, **`LITELLM_API_KEY`** — Bearer для backend-запросов `/v1/chat/completions`.
>
...
>
> **Версия 8.2.7:** в UI **«Отчёты логов»** режим **«Свой интервал»** использует поля `datetime-local` с календарём/выбором времени, автозаполняется последним часом и даёт быстрые кнопки **«Последний час»** / **«Последние 24 ч»**.
>
> **Версия 8.2.6:** **«Отчёты логов»** больше не падают целиком, если LiteLLM Proxy возвращает **500** на `/v1/chat/completions`: Loki-факты сохраняются, а `llm_markdown` заполняется локальной Markdown-сводкой по тем же агрегатам с предупреждением о недоступности LLM.
>
> **Версия 8.2.5:** на странице **«Пресеты»** добавлено скачивание выбранного пресета в `.env` на клиентский ПК: backend отдаёт **`GET /api/v1/presets/{id}/download-env`** как `attachment`, а UI сохраняет файл через браузер. Прежняя **«Выгрузить в PRESETS_DIR»** продолжает писать `.env` на серверный диск.
>
> **Версия 8.2.4:** в карточках пресетов поддержаны env-параметры vLLM **`TORCH_FLOAT32_MATMUL_PRECISION`** и **`VLLM_USE_V1`**. Они сохраняются в `presets.parameters`, импортируются из `.env`, экспортируются обратно и попадают в env контейнера vLLM без преобразования в CLI-флаги.
>
> **Версия 8.2.0:** в **slgpu-web** добавлен модуль **«Отчёты логов»** (SPA **`/log-reports`**): **`POST /api/v1/log-reports`** ставит фоновую задачу **`web.log_report.generate`** — контейнерные логи за период читаются из **Loki** (`query_range`), строится **детерминированный JSON «фактов»** (агрегации, топ ошибок, ограниченные примеры строк с редактированием секретных паттернов), затем best-effort вызов **LiteLLM** `POST …/v1/chat/completions` для **русской Markdown-сводки**; если LiteLLM недоступен, строится локальная Markdown-сводка по `facts`. Результат в SQLite **`log_reports`**, статус через **`GET /api/v1/log-reports/{id}`** (**202** при создании). Требует поднятого стека мониторинга (**Loki** доступен из контейнера web по **`LOKI_SERVICE_NAME`** / **`LOKI_INTERNAL_PORT`**). Подробности — [**`web/CONTRACT.md`**](web/CONTRACT.md).
>
...
>
> **Версия 7.0.0 (MAJOR):** из реестра и шаблона **`configs/main.env`** удалены **VLLM_HOST**, **VLLM_PORT**, **SGLANG_LISTEN_*** и на тот момент скрыт **LITELLM_MASTER_KEY**; listen LLM задаётся только **LLM_API_***. **Актуально с 8.2.11:** **`LITELLM_MASTER_KEY`** и **`LITELLM_API_KEY`** снова видимые registry-секреты блока **8. Секреты приложения**. Остальные детали этой большой миграционной заметки зафиксированы в [`docs/HISTORY.md`](docs/HISTORY.md).
...
**Отчёт по логам (UI «Отчёты логов»):** on-demand сводка за выбранный интервал через **Loki + LiteLLM** — см. [**`web/CONTRACT.md`**](web/CONTRACT.md) (таблица `log_reports`, API `/api/v1/log-reports`); нужен поднятый мониторинг. Если LiteLLM вернул 500 или недоступен, отчёт всё равно сохраняет `facts` и показывает локальную Markdown-сводку.
...
- **Стек мониторинга** (Prometheus / Grafana / Loki / …) — **`native.monitoring.*`**, страница **Мониторинг**; **прокси** (Langfuse + LiteLLM) — **`native.proxy.*`**, страница **LiteLLM Proxy** — независимые compose-файлы (**LiteLLM/proxy не требуют GPU**). **8.3.0:** **`dcgm-exporter`** в [`docker/docker-compose.monitoring.yml`](docker/docker-compose.monitoring.yml) в Compose profile **`gpu`**; ключ стека **`MONITORING_DCGM`** (`auto`|`on`|`off`, в [`configs/main.env`](configs/main.env) по умолчанию `auto`) управляет подъёмом DCGM, job `dcgm` в Prometheus и карточкой в UI — на CPU-only при `auto`/`off` метрики/логи работают без NVIDIA. **8.3.1:** **`HOST_GPU_DOCKER_PROBE`** (`on` по умолчанию; `off` — не выполнять эфемерный контейнер с **`nvidia-smi`** на хосте для UI/проб; остаётся локальный smi в web при наличии); **`NVIDIA_SMI_DOCKER_IMAGE`** — образ пробы (дефолт как у **`WEB_NVIDIA_SMI_DOCKER_IMAGE`**, `nvidia/cuda:12.4.1-base-ubuntu22.04`). На хосте: [`docker/docker-compose.monitoring.yml`](docker/docker-compose.monitoring.yml) и [`docker/docker-compose.proxy.yml`](docker/docker-compose.proxy.yml). Скрейп: [`configs/monitoring/prometheus/prometheus.yml`](configs/monitoring/prometheus/prometheus.yml). **Langfuse** + **LiteLLM** — [`configs/monitoring/README.md`](configs/monitoring/README.md) (`LANGFUSE_PORT` по умолч. 3001, `LITELLM_PORT`); **имя модели** в LiteLLM — как в **/ui** / **`SERVED_MODEL_NAME`**. Секреты — в `stack_params`, не в git.
...
| **SGLang (MiMo-V2.5 не Pro):** `MiMoV2 fused qkv_proj checkpoint is TP=4-interleaved; got attention tp_size=8` | Чекпоинт **TP=4-interleaved**; на 8×H200 нужны **`--tp 8`** + **`--dp 2`** + **`--enable-dp-attention`**, плюс **`--enable-dp-lm-head`** и **`--mm-enable-dp-encoder`** (см. [SGLang cookbook MiMo-V2.5](https://docs.sglang.io/cookbook/autoregressive/Xiaomi/MiMo-V2.5)). В репозитории: пресет [`mimo-v2.5-8.env`](examples/presets/mimo-v2.5-8.env) (`SGLANG_DP_SIZE`, `SGLANG_ENABLE_DP_ATTENTION`, …), образ **`lmsysorg/sglang:dev-mimo-v2.5`** |
...
| **vLLM data parallel** (`--data-parallel-size`): задать в **карточке пресета** | Ключ **`DATA_PARALLEL_SIZE`** (или алиас **`VLLM_DATA_PARALLEL_SIZE`** в `.env`); пусто — флаг не передаётся. **8.2.15:** глобальный стек больше не подмешивается (только пресет). Нужное число GPU: как в [рецептах vLLM](https://vllm.ai) (часто связка **TP + EP + DP**). |
