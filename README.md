...
| **vLLM (DeepSeek V4+):** `torch._inductor... InductorError` / `replace_by_example` / `profile_run` / `determine_available_memory` | **Убрать** агрессивный `SLGPU_VLLM_COMPILATION_CONFIG` в пресете (в [deepseek-v4-flash.env](configs/models/deepseek-v4-flash.env) по умолчанию **не** задаётся) или в [`main.env`](main.env) / пресете: **`SLGPU_VLLM_ENFORCE_EAGER=1`** → `--enforce-eager` (медленнее, обходит Inductor на проблемных связках torch+vLLM) |
