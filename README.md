...
Полный стек мониторинга и логов (Prometheus, Grafana, Loki, Promtail): сначала **`./slgpu monitoring fix-perms`**, чтобы каталоги `LOKI_DATA_DIR` / `PROMTAIL_DATA_DIR` и т.д. существовали с правами для контейнеров, затем **`./slgpu monitoring up`**; подробности — [configs/monitoring/README.md](configs/monitoring/README.md), логи — [configs/monitoring/LOGS.md](configs/monitoring/LOGS.md). `fix-perms` использует короткоживущий root-контейнер `docker run --rm -u 0:0` (образ из переменной `SLGPU_FIXPERMS_HELPER_IMAGE`, по умолчанию `alpine:latest`), поэтому **`sudo` не нужен** ни на хосте, ни внутри web-контейнера.
