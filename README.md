...
| **vLLM:** `RuntimeError: Unexpected error from cudaGetDeviceCount()… Error 803: system has unsupported display driver / cuda driver combination` | **Не драйвер**, а **capability** GPU-устройства в Docker. С v8.0.2 контейнер запускается с `DeviceRequest(capabilities=[["gpu","compute","utility"]])` и `NVIDIA_DRIVER_CAPABILITIES=compute,utility` — без `compute` `libcuda.so` пробрасывается без compute-режима, и `cudaGetDeviceCount()` возвращает 803. Если ошибка осталась после обновления — проверь, что **`nvidia-container-toolkit`** на хосте установлен (`nvidia-container-cli --version`) и `daemon.json` содержит `"runtimes": {"nvidia": …}`; драйвер `≥ 580` под CUDA 13 уже валиден. |
