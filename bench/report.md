...
**Период:** 21.04.2026, 5 прогонов Kimi-K2.6
**Железо:** стенд проекта (см. `configs/*`)
**Источник данных:** `bench/results/vllm/<timestamp>/` — `summary.json` + `time_series.csv` (для load-тестов).
...
В обоих прогонах отрабатывает одна и та же матрица 4×4: четыре шаблона нагрузки × четыре уровня параллелизма `{1, 8, 32, 128}`. В каждой ячейке — 1 раунд, т.е. `concurrency` одновременных стриминг-запросов, поэтому при малых `c` (1 и 8) выборка крошечная и p95 часто совпадает с p50.
...
Все 4 ячейки с **concurrency=1** провалены в обоих прогонах (по 1 ошибке на ячейку), все 4 ячейки с **max_tokens=256** провалены на всех уровнях параллелизма. На c=128 error-rate ≈ 90 %. Гипотезы по порядку правдоподобия:
...
2. **Сценарии scenario-матрицы** в текущем виде дают слишком много NaN. Что изменить:
   - убрать `concurrency=1` (или поднять порог warm-up/таймаут в `bench_openai.py`);
   - уменьшить `concurrency=128` до 64 и поднять серверный лимит потоков;
   - разобраться с сценариями `max_tokens=256` — проверить отдельно, почему они падают на всех c.
