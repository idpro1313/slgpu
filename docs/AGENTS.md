...
- **CLI (хост):** только **`./slgpu web up|down|restart|logs`**; env-файл — **`configs/main.env`** (обязателен). LLM/мониторинг/proxy — из **slgpu-web** (`native.*` jobs, Docker API), не из host `./slgpu`.
