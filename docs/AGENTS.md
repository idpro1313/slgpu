...
- **Стек в web:** **`stack_params`** + **`cfg.meta`**, операции мониторинга и бенча через **`native.*`** jobs (**docker compose** из backend). **Monitoring** и **proxy** — **разные** jobs (`native.monitoring.*` / `native.proxy.*`); общий снимок **`${WEB_DATA_DIR}/.slgpu/compose-service.env`** из БД (`sync_merged_flat`). См. [configs/monitoring/README.md](../configs/monitoring/README.md). **LiteLLM:** Bearer ключ задаётся только в блоке **«Внешний доступ»** (**`litellm_api_key`** в **`settings`**); в **`sync_merged_flat`** попадает в окружение как **`LITELLM_MASTER_KEY`** (**нет ключа в реестре/stack_params с v7**).
