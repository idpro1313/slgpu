...
- **Стек в web:** **`stack_params`** + **`cfg.meta`**, операции мониторинга и бенча через **`native.*`** jobs (**docker compose** из backend). **Monitoring** и **proxy** — **разные** jobs (`native.monitoring.*` / `native.proxy.*`); общий снимок **`${WEB_DATA_DIR}/.slgpu/compose-service.env`** из БД (`sync_merged_flat`). См. [configs/monitoring/README.md](../configs/monitoring/README.md). **`8.1.6`–`8.1.7`:** второй Prometheus job **`vllm-slots`** (file_sd) читает **`${WEB_DATA_DIR}/.slgpu/monitoring/vllm-slots.json`** — дополнительные хост-порты vLLM при мультислоте из UI; `render_monitoring_configs` при **отсутствии** файла создаёт дефолт **`8110–8130`** (**8.1.7**), существующий файл не перезаписывается. **LiteLLM:** Bearer ключ задаётся только в блоке **«Внешний доступ»** (**`litellm_api_key`** в **`settings`**); в **`sync_merged_flat`** попадает в окружение как **`LITELLM_MASTER_KEY`** (**нет ключа в реестре/stack_params с v7**), а с **8.2.9** `write_compose_service_env_file` сохраняет этот derived-key в `compose-service.env` несмотря на отсутствие в `CANONICAL_STACK_KEYS`.
