...
- **CLI:** единая точка входа `./slgpu` (bash); LLM-стек — `docker/docker-compose.llm.yml`; мониторинг — каталог **`configs/monitoring/`**, `docker/docker-compose.monitoring.yml`, **LiteLLM** — отдельно `docker/docker-compose.proxy.yml` (проект `slgpu-proxy`), тот же **`./slgpu monitoring up|down|restart`**. Первый `monitoring up` — bootstrap в monitoring (`minio-bucket-init`, `litellm-pg-init`; маркеры в `data/monitoring/.bootstrap`), затем подъём proxy. **Только прокси** в web: **`POST /api/v1/litellm/proxy/action`** → `native.proxy.up|down|restart` (lock с monitoring); UI — страница **LiteLLM**.
...
- **Стек в web:** **`stack_params`** + **`cfg.meta`**, операции мониторинга и бенча через **`native.*`** jobs. **Важно:** **`./slgpu monitoring`** читает **`main.env`**, а не SQLite; несоответствие портов web vs файл — см. [configs/monitoring/README.md](../configs/monitoring/README.md) (раздел «Порты: настройки web и docker compose»). Ключ **LiteLLM** из **«Публичный доступ»** (`litellm_api_key`) в **`sync_merged_flat`** маппится в **`LITELLM_MASTER_KEY`** для native compose; **bash monitoring** — только `main.env`.
