...
- **Стек в web:** **`stack_params`** + **`cfg.meta`**, операции мониторинга и бенча через **`native.*`** jobs. **Monitoring/proxy:** единый снимок **`.slgpu/compose-service.env`** (из **БД** в native jobs; в bash — копия **`main.env`** при наличии). См. [configs/monitoring/README.md](../configs/monitoring/README.md). Ключ **LiteLLM** из **«Публичный доступ»** (`litellm_api_key`) в **`sync_merged_flat`** маппится в **`LITELLM_MASTER_KEY`**.
