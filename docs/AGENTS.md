...
- **CLI:** единая точка входа `./slgpu` (bash); LLM-стек — `docker/docker-compose.llm.yml`; **метрики/логи** — `docker/docker-compose.monitoring.yml` и каталог **`configs/monitoring/`**; **Langfuse + LiteLLM** — `docker/docker-compose.proxy.yml` (проект `slgpu-proxy`), тот же **`./slgpu monitoring up|down|restart`**. Первый `monitoring up` — bootstrap в **proxy** (`minio-bucket-init`, `litellm-pg-init`; маркеры в `data/monitoring/.bootstrap`), затем подъём `monitoring` (Prometheus/…) и `proxy` (Langfuse, LiteLLM). **Только прокси** в web: **`POST /api/v1/litellm/proxy/action`** → `native.proxy.up|down|restart` (lock с monitoring); UI — страница **LiteLLM**.
- **Web UI — Develonica.LLM:** **`./slgpu web up`** (без `main.env` в корне: **`docker/web-compose.defaults.env`**, дефолты в `docker-compose.web.yml`); импорт стека: **`/api/v1/app-config/install`** — файл **`main.env`** в корне репо обязателен. Визуал в духе [`develonica.ru`](https://develonica.ru/) / [гайдлайна](https://develonica.ru/company/guideline/): **`IBM Plex Sans`**, **`Finlandica`** для заголовков и чисел, голубая палитра **`#59AFFF`** / hover **`#0A5AA4`**, светлые градиенты, pill-навигация, SVG favicon; **плотный макет** (`globals.css`: меньше отступов/кегль, кнопки и секции компактнее); footer — версия из корневого **`VERSION`** через **`/healthz`**, копирайт `Igor Yatsishen, Develonica`.
...
- **Модели и пресеты:** реестр в БД синхронизируется с **`MODELS_DIR/<org>/<repo>`**; **`pull_progress`** в **`GET /api/v1/models`** — активная **`native.model.pull`**. Пресеты: рабочие **`data/presets/*.env`**, эталоны **`examples/presets/`**, формат — **`configs/models/README.md`**; **UI** — CRUD, клон, **«Выгрузить в .env»** из карточки (`POST /presets/{id}/export`); **`POST /presets/sync`** и **`/import-templates`** — без кнопок в SPA (хост/скрипты). **Логи Docker (все слоты/мониторинг):** **`GET /api/v1/docker/containers`**, **`…/docker/containers/{id|name}/logs`**, **`/docker/engine-events`**, **`/docker/daemon-log`**; UI **`/docker-logs`**.
...
- **Стек в web:** **`stack_params`** + **`cfg.meta`**, операции мониторинга и бенча через **`native.*`** jobs (без вызова `./slgpu`; **docker compose** из backend). **Monitoring/proxy:** снимок **`${WEB_DATA_DIR}/.slgpu/compose-service.env`** (по умолч. `data/web/…`, пишется из **БД**; на хосте без web — **`main.env`** → тот же путь через **`slgpu_ensure_compose_service_env`**). См. [configs/monitoring/README.md](../configs/monitoring/README.md). Ключ **LiteLLM** из **«Публичный доступ»** (`litellm_api_key`) в **`sync_merged_flat`** маппится в **`LITELLM_MASTER_KEY`**.
