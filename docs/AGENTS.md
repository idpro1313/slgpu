...
- **Стек в web:** **`stack_params`** + **`cfg.meta`**, операции мониторинга и бенча через **`native.*`** jobs. **Важно:** **`./slgpu monitoring`** читает **`main.env`**, а не SQLite; несоответствие портов web vs файл — см. [configs/monitoring/README.md](../configs/monitoring/README.md) (раздел «Порты: настройки web и docker compose»).
