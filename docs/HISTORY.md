...
- **Что:** В **`web/frontend/src/pages/Settings.tsx`** у **`SCOPE_LABELS.monitoring_up`** строка **`"монитори&shy;нг"`** заменена на **`"мониторинг"`**. HTML-условный перенос в тексте React не интерпретируется как в HTML и давал странное отображение («монитори&shy;нг» в списке «обязателен для: …»).
- **Почему:** Запрос пользователя про «странный комментарий» к параметрам.
- **Файлы:** **`VERSION` 6.1.2**, синхронизация версий web, **`README.md`**, **`grace/knowledge-graph/knowledge-graph.xml`**.
...
- **Что:** Расширён **`_RUNTIME_KEYS`** в `web/backend/app/services/presets.py`: **`SGLANG_DOCKER_IMAGE`**, **`SGLANG_ENABLE_SPEC_V2`**, **`SGLANG_TRUST_REMOTE_CODE`**, **`SGLANG_ENABLE_METRICS`** / **`SGLANG_ENABLE_MFU_METRICS`**, DP-attention (**`SGLANG_DP_SIZE`**, **`SGLANG_ENABLE_DP_ATTENTION`**, **`SGLANG_ENABLE_DP_LM_HEAD`**, **`SGLANG_MM_ENABLE_DP_ENCODER`**), **`SGLANG_CHUNKED_PREFILL_SIZE`**, блок EAGLE (**`SGLANG_SPECULATIVE_*`**, **`SGLANG_ENABLE_MULTI_LAYER_EAGLE`**), **`NVIDIA_VISIBLE_DEVICES`** — ключи не отбрасываются при **`presentation_preset_parameters`** / импорте `.env`. В **`Presets.tsx`** расширен **`COMMON_PARAMETERS`**. В **`env_files.render_env_text`** — **`SGLANG_DOCKER_IMAGE`** в Identity и расширенный блок SGLang; **`NVIDIA_VISIBLE_DEVICES`** в Runtime. В **`stack_registry`** — описания новых inference-ключей. Обновлены **`README`**, **`docs/AGENTS.md`**, версии web.
- **Почему:** Расширенные ключи SGLang не были в **`COMMON_PARAMETERS`**; без канонизации в **`_RUNTIME_KEYS`** они **терялись** при импорте/сохранении пресета (в т.ч. эталон **`examples/presets/mimo-v2.5-8.env`**).
