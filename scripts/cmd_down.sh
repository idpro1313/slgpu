...
if [[ "${ALL}" -eq 1 ]]; then
  ...
  slgpu_docker_compose -f docker/docker-compose.proxy.yml --env-file "${_ce}" stop 2>/dev/null || true
  slgpu_docker_compose -f docker/docker-compose.monitoring.yml --env-file "${_ce}" stop 2>/dev/null || true
