...
usage() {
  cat <<EOF
Мониторинг (метрики и логи): dcgm-exporter, node-exporter, Prometheus, Grafana, **Loki**, **Promtail** — `docker/docker-compose.monitoring.yml` (проект **slgpu-monitoring**). **Langfuse** (UI + Postgres/MinIO/…) и **LiteLLM** (OpenAI → vLLM) — `docker/docker-compose.proxy.yml` (проект **slgpu-proxy**); сеть `slgpu` та же. Логи контейнеров: **Grafana → Explore → Loki**. Langfuse: `http://<хост>:$LANGFUSE_PORT` (по умолч. 3001), LiteLLM: LITELLM_PORT (по умолч. 4000); модели/маршруты — `configs/monitoring/litellm/config.yaml` и /ui (часто **devllm** = `SERVED_MODEL_NAME`).

Один раз на хост (или после reboot, если не включён restart: unless-stopped):
  ./slgpu monitoring up
  (первый up: bootstrap MinIO + БД litellm в postgres — в **proxy** compose; затем метрики **monitoring** + **proxy** Langfuse+LiteLLM)
...
Конфиг: `docker/docker-compose.monitoring.yml` (Prometheus/Grafana/Loki/…) + `docker/docker-compose.proxy.yml` (Langfuse + LiteLLM); сеть `slgpu` — общая с `docker/docker-compose.llm.yml`.
...
slgpu_ensure_langfuse_litellm_secrets() {
  # С compose: data/web/… (Langfuse/LiteLLM — proxy compose). Устаревший configs/secrets/ — только с хоста вручную.
...
slgpu_monitoring_run_bootstrap_service() {
  ...
  slgpu_docker_compose -f docker/docker-compose.proxy.yml --env-file "${_ce}" --profile bootstrap rm -f -s -v "${svc}" >/dev/null 2>&1 || true
  slgpu_docker_compose -f docker/docker-compose.proxy.yml --env-file "${_ce}" --profile bootstrap up --abort-on-container-exit --exit-code-from "${svc}" "${svc}"
  slgpu_docker_compose -f docker/docker-compose.proxy.yml --env-file "${_ce}" --profile bootstrap rm -f -s -v "${svc}" >/dev/null 2>&1 || true
  touch "${marker}"
...
case "${SUB}" in
  up)
    ...
    slgpu_ensure_monitoring_bind_config_files
    slgpu_monitoring_bootstrap_once
    echo "Поднимаю стек метрик и логов (slgpu-monitoring)…"
    slgpu_docker_compose -f docker/docker-compose.monitoring.yml --env-file "${_ce}" up -d --remove-orphans
    echo "Поднимаю Langfuse + LiteLLM (slgpu-proxy)…"
  ...
  down)
    ...
    slgpu_ensure_compose_service_env
    _ce="$(slgpu_compose_service_env_basename)"
    echo "Останавливаю Langfuse + LiteLLM (slgpu-proxy)…"
  ...
  restart)
    ...
    echo "Перезапуск Langfuse + LiteLLM (slgpu-proxy)…"
